...
submission/                SUBMISSION.md (the kit: form fields, deliverables, X post source text),
                           PROJECT-DESCRIPTION-{EN,CN}.txt (paste-ready, generated by
                           `npm run form:text`), VIDEO-FACTS.json (measured by `npm run video:probe`).
                           The paste-ready X posts (`npm run xpost`) and the author's task list are
                           gitignored: submission logistics stay out of the reviewed tree.
```
