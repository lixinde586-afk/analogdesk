...
// The reference-session convention and the closed-session return layer are shared with
// scripts/measure-bitget-7x24.mjs on purpose: two venues are only comparable if both were measured
// with one definition of "the cash market was closed", one minimum block length and one entry/exit
// rule. Editing a rule here now edits it for both venues, which is the point.
import {
  OPEN_HOURS_UTC, OPEN_DAYS_UTC, CASH_HOURS_PER_WEEK, WEEK_HOURS,
  MIN_BLOCK_HOURS, WEEKEND_BLOCK_HOURS, SESSION_CONVENTION, CLOSED_RETURN_CONVENTION,
  stdev, r, r0, isReferenceOpen, closedHours, closedReturns, pooledDistribution, ACCEPTANCE
...
/** Acceptance thresholds, shared with the Bitget venue so the two are comparable. Asserted by
 *  scripts/check-wrapper.mjs against the committed file, so they cannot drift silently. */
export const { TOL_PCT, MIN_CORR, MIN_OVERLAP, MAX_CANDIDATE_SUFFIX } = ACCEPTANCE;
